...
# 🚀 TOPOLOGY - LOAD TOPOLOGY CONFIGURATION
   ...
   kubectl exec -ti -n $AIOPS_NAMESPACE $(oc get po -n $AIOPS_NAMESPACE|grep topology-topology|awk '{print$1}'|head -1) -- /opt/ibm/graph.tools/bin/backup_ui_config -force -out backup.json
   ...
   #open ./backup.json
    ...
    if [[ "${OS}" == "darwin" ]]; then
          echo "MAC"
          TOPOLOGY_CUSTOM_FILE=$(pwd)"/ansible/roles/ibm-concert-deployer-demo-content/templates/topology/asm_config_46.json"
    else
          TOPOLOGY_CUSTOM_FILE="./ansible/roles/ibm-concert-deployer-demo-content/templates/topology/asm_config_46.json"
    fi    
