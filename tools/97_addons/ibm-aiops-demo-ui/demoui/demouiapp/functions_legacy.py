...
ROBOTSHOP_PROPERTY_RESOURCE_NAME = os.environ.get(
    "ROBOTSHOP_PROPERTY_RESOURCE_NAME", "mysql"
)
ROBOTSHOP_PROPERTY_RESOURCE_TYPE = os.environ.get(
    "ROBOTSHOP_PROPERTY_RESOURCE_TYPE", "deployment"
)
ROBOTSHOP_PROPERTY_VALUES_NOK = os.environ.get(
    "ROBOTSHOP_PROPERTY_VALUES_NOK",
    '{"innodb_buffer_pool_size": "8GB","last_change_by": "Niklaus Hirt"}',
)
ROBOTSHOP_PROPERTY_VALUES_OK = os.environ.get(
...
METRICS_TO_SIMULATE_NET_SOCK = str(
...
# GLOBAL
METRIC_TIME_SKEW = int(os.environ.get("METRIC_TIME_SKEW", "240"))
METRIC_TIME_STEP = int(os.environ.get("METRIC_TIME_STEP", "100"))
...
EVENTS_TIME_SKEW = int(os.environ.get("EVENTS_TIME_SKEW", "10"))
...
def closeStories(DATALAYER_ROUTE, DATALAYER_USER, DATALAYER_PWD):
    ...
    try:
        response = requests.patch(
            url, data=dataInProgress, headers=headers, auth=auth, verify=False
        )
    except requests.exceptions.RequestException as e:  # This is the correct syntax
        stream = capture_shell(
            "oc get route  -n "
    ...
    try:
        response = requests.patch(
            url, data=dataResolved, headers=headers, auth=auth, verify=False
        )
    except requests.exceptions.RequestException as e:  # This is the correct syntax
        stream = capture_shell(
            "oc get route  -n "
...
def injectMetrics(
    ...
    stream = capture_shell(
        'curl -s -k -H "Content-Type: application/x-www-form-urlencoded;charset=UTF-8" -d "grant_type=password&username='
        + tmpusr
        + "&password="
        + tmppass
        + '&scope=openid" https://'
    ...
    stream = capture_shell(
        "curl -s -k -XGET https://"
        + METRIC_ROUTE
        + '/v1/preauth/validateAuth -H "username: '
        + tmpusr
        + '" -H "iam-token: '
        + ACCESS_TOKEN
        + '"|jq -r ".accessToken"'
    ...
    # stream = capture_shell("oc get secret  -n "+aimanagerns+" admin-user-details -o jsonpath='{.data.initial_admin_password}' | base64 --decode")
    # tmppass = stream.read().strip()
    # stream = capture_shell('curl -k -s -X POST https://'+METRIC_ROUTE+'/icp4d-api/v1/authorize -H "Content-Type: application/json" -d "{\\\"username\\\": \\\"admin\\\",\\\"password\\\": \\\"'+tmppass+'\\\"}" | jq .token | sed "s/\\\"//g"')
    # METRIC_TOKEN = stream.read().strip()
