...
As the Slack integration requires a two-way communication and for Slack to communicate with the Concert Operate server, you need to configure a proxy. Here, you will use IBM Secure Gateway as the proxy.
...
1. Select the region closest to you `Dallas` (for example), select the `free tier`, use your email-id for organization and select a space, for example: `dev` and click the `Create` button.
...
1. The service will be listed under Cloud Foundry Services. Open the service. Now, you can add a Gateway. Ensure that you uncheck the `Token Expiration`

   ![secure_gw_add](images/securegw_add.png) and then ![secure_gw_add1](images/securegw_add1.png)

1. Once the gateway is added, you need to get the `Gateway ID` and the `token`. For this, click on the gear icon in the created gateway and make a note of security token and the Gateway ID.
...
6. Input `ibm-nginx-svc.ibm-aiops.svc.cluster.local` and `443`
   ...
   ❗ Replace `ibm-aiops` with the name of your Concert Operate namespace if you have changed it in `01_config-install.sh`.
...
13. Name your destination whatever you like
...
3. This will be the first part of the URL (e.g. cap-eu-de-prd-sg-bm-02.securegateway.appdomain.cloud:15145)
4. Keep the second part from the integration URL (e.g. /aiops/aimanager/instances/xxxxx/api/slack/events)

5. This will give you the final URL (e.g. `https://cap-eu-de-prd-sg-bm-02.securegateway.appdomain.cloud:15145/aiops/aimanager/instances/xxxxx/api/slack/events)

6. Paste the URL into a browser and you should get `Method Not Allowed`
